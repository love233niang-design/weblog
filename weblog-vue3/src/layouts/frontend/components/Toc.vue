<template>
    ...
    <div v-if="titles && titles.length > 0"
        class="sticky top-24 text-sm/[30px] w-full blog-card p-5">
        <h2 class="blog-section-title mb-3">目录</h2>
        <div class="toc-wrapper" :class="[isDark ? 'dark' : '']">
            <ul class="toc">
                <li v-for="(h2, index) in titles" :key="index">
                    <span @click="scrollToView(h2.offsetTop)" class="pl-4 hover:text-accent cursor-pointer transition-colors"
                        :class="[h2.index == activeHeadingIndex ? 'active py-1 text-accent border-l-2 border-accent font-semibold dark:text-accent-muted dark:border-accent-muted' : 'text-ink-500 font-normal']">{{
                        h2.text }}</span>
                    <ul v-if="h2.children && h2.children.length > 0">
                        <li v-for="(h3, index2) in h2.children" :key="index2">
                            <span @click="scrollToView(h3.offsetTop)" class="pl-8 hover:text-accent cursor-pointer transition-colors"
                                :class="[h3.index == activeHeadingIndex ? 'active py-1 text-accent border-l-2 border-accent font-semibold dark:text-accent-muted dark:border-accent-muted' : 'text-ink-500 font-normal']">{{
                                h3.text }}</span>
                        </li>
                    </ul>
                </li>
