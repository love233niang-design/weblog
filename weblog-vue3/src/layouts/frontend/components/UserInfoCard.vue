    ...
    <div class="blog-card p-6">
        <div class="flex flex-col items-center text-center">
            <div class="relative mb-4">
                <img class="w-16 h-16 rounded-full object-cover ring-4 ring-accent-soft/60 dark:ring-accent-dark/50"
                    :src="blogSettingsStore.blogSettings.avatar" alt="avatar" />
                <span
                    class="absolute bottom-0.5 right-0.5 w-3.5 h-3.5 bg-emerald-400 border-2 border-white dark:border-ink-900 rounded-full"></span>
            </div>

            <h5 class="font-display text-lg font-semibold text-ink-900 dark:text-white">
                {{ blogSettingsStore.blogSettings.author }}
            </h5>
            <p class="mt-1.5 mb-6 text-sm text-ink-500 dark:text-ink-400 leading-relaxed max-w-[16rem]"
            ...
            <div id="introduction-tooltip-bottom" role="tooltip"
                class="absolute z-10 invisible inline-block px-3 py-2 text-xs font-medium text-white bg-ink-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-ink-700">
                介绍语
                <div class="tooltip-arrow" data-popper-arrow></div>
            </div>

            <div class="grid grid-cols-4 gap-2 w-full mb-5">
                <div class="flex flex-col items-center gap-0.5 py-2 rounded-xl hover:bg-ink-50 dark:hover:bg-ink-800/60 transition-colors cursor-pointer group">
                    <CountTo :value="statisticsInfo.articleTotalCount"
                        customClass="text-base font-semibold text-ink-800 dark:text-ink-100 group-hover:text-accent"></CountTo>
                    <div class="text-[11px] text-ink-400">文章</div>
                </div>
                <div class="flex flex-col items-center gap-0.5 py-2 rounded-xl hover:bg-ink-50 dark:hover:bg-ink-800/60 transition-colors cursor-pointer group">
                    <CountTo :value="statisticsInfo.categoryTotalCount"
                        customClass="text-base font-semibold text-ink-800 dark:text-ink-100 group-hover:text-accent"></CountTo>
                    <div class="text-[11px] text-ink-400">分类</div>
                </div>
                <div class="flex flex-col items-center gap-0.5 py-2 rounded-xl hover:bg-ink-50 dark:hover:bg-ink-800/60 transition-colors cursor-pointer group">
                    <CountTo :value="statisticsInfo.tagTotalCount"
                        customClass="text-base font-semibold text-ink-800 dark:text-ink-100 group-hover:text-accent"></CountTo>
                    <div class="text-[11px] text-ink-400">标签</div>
                </div>
                <div class="flex flex-col items-center gap-0.5 py-2 rounded-xl">
                    <CountTo :value="statisticsInfo.pvTotalCount"
                        customClass="text-base font-semibold text-ink-800 dark:text-ink-100"></CountTo>
                    <div class="text-[11px] text-ink-400">访问</div>
                </div>
            </div>

            <div class="flex justify-center gap-2.5" v-if="hasSocial">
                <svg @click="jump(blogSettingsStore.blogSettings.githubHomepage)"
                    ...
                    class="hover:scale-110 transition-transform cursor-pointer icon w-7 h-7" viewBox="0 0 1024 1024"
                ...
                <div id="zhihu-tooltip-bottom" role="tooltip"
                    class="absolute z-10 invisible inline-block px-3 py-2 text-xs rounded-lg font-medium text-white bg-ink-900 shadow-sm opacity-0 tooltip dark:bg-ink-700">
                    我的知乎
                    <div class="tooltip-arrow" data-popper-arrow></div>
                </div>
